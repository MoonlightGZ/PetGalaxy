...
export function Button({ className, variant = "primary", ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary" | "secondary" | "ghost" | "outline" }) {
  const variants = {
    ...
    outline: "border border-slate-200/80 bg-white/50 dark:border-white/10 dark:bg-white/5"
  };
...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <section className={cn("glass rounded-[2rem] p-6", className)} {...props} />;
}
...
export const inputClass = "focus-ring w-full rounded-2xl border border-slate-200/80 bg-white/70 px-4 py-3 text-sm text-slate-950 shadow-inner shadow-slate-900/5 placeholder:text-slate-400 dark:border-white/10 dark:bg-white/10 dark:text-white";
